...
const getNames = (arr) => {
  // Solution code here...
  let reversedItems =arr.map((item ) => {
    // console.log(item);
    let x=item.name.split('');
    let z= x.reverse();
    let s= z.join('');
    // let e = s.reverse();
    return s;
...
const count = (target, input) => {
  let countt = 0;
  let newArray = input.map(arr=>{
    arr.map(num =>{ if(num === target){
...
const totalSum = (input) => {
  let arr= input.reduce( ( sum, num) => sum.concat(num),
    []
  );
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(val =>{
    arrnum = val.filter(val => val%5 === 0 && typeof val === 'number');
    arrnum.forEach((val, i )=> arrnum[i]= Math.pow(2,val));
    newArr.push(arrnum);
  });
  return newArr;
};
...
findMaleAndFemale([{ name: 'person', gender: 'female' }, { gender: 'lol' }, { name: 'persontwo', gender: 'male' }])).
------------------------------------------------------------------------------------------------ */
...
let findMaleAndFemale = (data) => {
  ...
  // return arr;
  let starNames = arr.map(p=>
  {
    return p.name;
    // console.log(p.name);
  });
  let sWars = starNames.join( ' and ' );
  // console.log(sWars);
  return sWars;
};
...
let findShortest = (data) => {
  let arr=data.filter(names => {
    if(names.gender!=='female' & names.gender!=='male' & names.name !=='C-3PO')
    {
      return names.name;
    }
  });
